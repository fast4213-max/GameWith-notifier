...
ただし通知済みのタイトルが「未定」等（数字を含まない表記）に変わった場合は延期とみなし、⏳付きで変更前の日付と一緒に通知する。その後日付が決まれば🔁で再通知する。
...
- 1回の実行あたりの通知上限は30件（`config.MAX_NOTIFY_PER_RUN`）。10件ずつ1メッセージにまとめて送信し（最大3通）、Discordのレート制限ヘッダーに従って間隔を空ける。超過分は破棄せず`state/*.json`内の`queue`に積んで次回実行時に優先送信する。
